...
export function usePrep(device, language, modelSize) {
  const [prep, setPrep] = useState({ phase: "idle", message: "", pct: null });
  const [downloadId, setDownloadId] = useState(null);
  const r = useRef({ pf: null, pt: null, sourceId: null, urlKey: null, abort: null, lang: language === "auto" ? null : language, modelSize });

  useEffect(() => {
    r.current.lang = language === "auto" ? null : language;
    r.current.modelSize = modelSize;
  }, [language, modelSize]);

  const clear = () => { 
    clearTimeout(r.current.pf); clearTimeout(r.current.pt); 
    r.current.pf = r.current.pt = null; 
    if (r.current.abort) {
  ...
  async function pollPrefetch(id) {
    try {
      ...
      if (s.status === "downloading") { setPrep({ phase: "downloading", message: "Downloading video… " + mb(s.downloaded) + "MB", pct: s.total ? Math.round(s.downloaded / s.total * 100) : null }); r.current.pf = setTimeout(() => pollPrefetch(id), 500); }
      else if (s.status === "processing") { setPrep({ phase: "downloading", message: "Processing media format…", pct: null }); r.current.pf = setTimeout(() => pollPrefetch(id), 500); }
      else if (s.status === "error") setPrep({ phase: "error", message: "Download failed: " + s.error, pct: null });
      else if (s.status === "done") { setDownloadId(s.download_id); startTranscribe(s.download_id, r.current.lang, r.current.modelSize); }
    } catch {}
  }
  ...
  async function pollTranscribe(id) {
    try {
      ...
      else if (s.status === "error") setPrep({ phase: "error", message: "Transcription failed: " + s.error, pct: null });
  ...
  async function startTranscribe(sourceId, l, ms) {
    clearTimeout(r.current.pt);
    if (r.current.abort) {
      r.current.abort.abort();
    }
    r.current.abort = new AbortController();
    r.current.sourceId = sourceId;
    setPrep({ phase: "transcribing", message: "Preparing transcript…", pct: 0 });
    try {
      const s = await api.pretranscribe(sourceId, device, l, ms, r.current.abort.signal);
  ...
  function startUrl(url) {
    ...
    api.prefetch(url).then((s) => {
      if (s.status === "done") { setDownloadId(s.download_id); startTranscribe(s.download_id, r.current.lang, r.current.modelSize); }
  ...
  function startUpload(uploadId) {
    ...
    clear(); setDownloadId(null);
    startTranscribe(uploadId, r.current.lang, r.current.modelSize);
  }

  // Re-prepare the transcript in a newly chosen language (download is reused).
  function relang(newLang) {
    if (r.current.sourceId) startTranscribe(r.current.sourceId, newLang === "auto" ? null : newLang, modelSize);
  }

  // Re-prepare if the user changes the model size.
  function retranscribe(newModelSize) {
    if (r.current.sourceId) startTranscribe(r.current.sourceId, r.current.lang, newModelSize);
  }

  useEffect(() => () => clear(), []);
  return { prep, downloadId, startUrl, startUpload, relang, retranscribe, reset };
}
