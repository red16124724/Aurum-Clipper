<#
  ...
  build_portable.ps1  -  Aurum Clipper v1 portable-ZIP builder (Windows, GPU + CPU)
...
$Bundle = Join-Path $OutDir "AurumClipper"
...
REM ===== Aurum Clipper - double-click to run (koi install nahi) =====
...
echo   Aurum Clipper shuru ho rahi hai... (Made by RED4724) browser khud khul jayega.
...
Masla aaye to:
  ...
  - Port 8000 busy ho to koi aur Aurum Clipper/uvicorn pehle se chal raha hoga.

Banaya: The RED4724 - Made by RED4724 - 100% local pipeline (yt-dlp - faster-whisper - ffmpeg)
...
$zipPath = Join-Path $OutDir "AurumClipper-v1-portable.zip"
